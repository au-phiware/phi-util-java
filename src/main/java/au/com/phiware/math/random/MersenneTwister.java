package au.com.phiware.math.random;
...
*/
public abstract class MersenneTwister<T extends Number> extends RandomEngine<T> {
	private int mti;
...
/**
 * Constructs and returns a random number generator with a default seed, which is base on the current time.
 */
...
private MersenneTwister(Integer seed) {
	if (seed != null)
		setSeed(seed);
	else
...
public static class Builder<T extends Number> implements RandomEngine.Builder<T> {
	private Integer seed = null;
	private Class<T> type;
	private final static Map<Class<? extends Number>, RandomEngine<? extends Number>> instances = new HashMap<>();
	...
	@Override public RandomEngine<T> build() {
		if (seed == null) {
			if (!instances.containsKey(type)) {
				if (type.isAssignableFrom(Double.class))
					instances.put(type, new MersenneTwister<Double>(seed) {
						@Override public Double get() {
							return nextDouble();
						}
					});
				else if (type.isAssignableFrom(Float.class))
					instances.put(type, new MersenneTwister<Float>(seed) {
						@Override public Float get() {
							return nextFloat();
						}
					});
				else if (type.isAssignableFrom(Integer.class))
					instances.put(type, new MersenneTwister<Integer>(seed) {
						@Override public Integer get() {
							return nextInt();
						}
					});
				else if (type.isAssignableFrom(Long.class))
					instances.put(type, new MersenneTwister<Long>(seed) {
						@Override public Long get() {
							return nextLong();
						}
					});
			}
			return (RandomEngine<T>) instances.get(type);
		}
		if (type.isAssignableFrom(Double.class))
			return (RandomEngine<T>) new MersenneTwister<Double>(seed) {
				@Override public Double get() {
					return nextDouble();
				}
			};
		if (type.isAssignableFrom(Float.class))
			return (RandomEngine<T>) new MersenneTwister<Float>(seed) {
				@Override public Float get() {
					return nextFloat();
				}
			};
		if (type.isAssignableFrom(Integer.class))
			return (RandomEngine<T>) new MersenneTwister<Integer>(seed) {
				@Override public Integer get() {
					return nextInt();
				}
			};
		if (type.isAssignableFrom(Long.class))
			return (RandomEngine<T>) new MersenneTwister<Long>(seed) {
				@Override public Long get() {
					return nextLong();
				}
			};
		throw new UnsupportedOperationException("This random engine can only suply Double, Float, Integer or Long.");
	}

	@Override public RandomEngine<T> build(int seed) {
		this.seed = seed;
		return build();
	}

	@Override public RandomEngine<T> build(Date seed) {
		this.seed = (int) seed.getTime();
		return build();
	}
}

public static <T extends Number> Builder<T> builder(Class<T> type) {
	return new MersenneTwister.Builder<T>(type);
}
